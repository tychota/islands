defmodule IslandsEngine.Game do
  ...
  def handle_call({:position_island, player, key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:position_islands, player}),
      ...
      :error -> state_data |> reply_error
      {:error, :invalid_coordinate} -> state_data |> reply_error(:invalid_coordinate)
      {:error, :invalid_island_type} -> state_data |> reply_error(:invalid_island_type)
    end
  end
  ...
  def handle_call({:set_islands, player}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:set_islands, player}),
      ...
      :error -> state_data |> reply_error
      false -> state_data |> reply_error(:not_all_islands_positioned)
    end
  end
  ...
  def handle_call({:guess_coordinate, player_key, row, col}, _from, state_data) do
    ...
    with {:ok, rules} <- Rules.check(state_data.rules, {:guess_coordinate, player_key}),
      ...
      :error ->
        state_data |> reply_error

      {:error, :invalid_coordinate} ->
        state_data |> reply_error(:invalid_coordinate)
    end
  end
  ...
  defp reply_error(state_data, error) when is_atom(error),
    do: {:reply, {:error, error}, state_data, @timeout}

  defp reply_error(state_data, error) when is_nil(error),
